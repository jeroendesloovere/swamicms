...
class route
{
	...
	// Arguments
	// ---------------------------------------------------------------------------
	public static function arguments($key=false)
	{
		if(is_int($key)&&isset(self::$current['arguments'][(int)$key])) return self::$current['arguments'][(int)$key];
		elseif($key===false) return self::$current['arguments'];
		else return false;
	}
}