...
// @date:			14-06-2011
// @description:	Returns clean data for output or input
// ===========================================================================
class clean
{
	// Method cache array
 ...
 *
 * {{{
 * clean::rules('plural', array('/^(inflect)or$/i' => '\1ables'));
 * clean::rules('plural', array(
 *     'rules' => array('/^(inflect)ors$/i' => '\1ables'),
 *     'uninflected' => array('dontinflectme'),
 *     'irregular' => array('red' => 'redlings')
 * ));
 * clean::rules('transliteration', array('/å/' => 'aa'));
 * }}}
 *
 ...
 */
	...
	// Meta
	// ---------------------------------------------------------------------------
	public static function meta($meta, $separator = ', ', $allowUppercase = true, $convertCharacters = false)
	{
		return self::output($meta, $separator, false);
	}
	
	// Slug
	// ---------------------------------------------------------------------------
	public static function slug($slug, $separator = '-', $allowUppercase = false)
	{
		// Replace special chars by safe characters
		...
		$map = self::$_special_characters + $merge;
		$slug = preg_replace(array_keys($map), array_values($map), $slug);
		
		// Allow uppercase?
		if(!$allowUppercase) $slug = strtolower($slug);
	...
	// Camilize
	// ---------------------------------------------------------------------------
	public static function camilize($lowerCaseAndUnderscoredWord)
	{
		if (!($result = self::_cache(__FUNCTION__, $lowerCaseAndUnderscoredWord))) {
	...
	// Filename
	// ---------------------------------------------------------------------------
	public static function filename($fname, $separator = '-')
	{
		// Get filename slug => before last occurrence of "."
		$filename = self::slug(data::getBeforeLast('.', $fname), $separator);
		
		// Get extension => after last occurrence of "."
		$extension = strtolower(data::getAfterLast('.', $fname));
	...
	// Foldername
	// ---------------------------------------------------------------------------
	public static function foldername($foldername, $separator = '-')
	{
		return self::slug($foldername, $separator);
	}
	
	// Clean output data
	// ---------------------------------------------------------------------------
	public static function output($str, $separator = ' ', $allowTags = true)
	{
		// Make string
		$str = data::toString($str, $separator);
		
		// Allow tags?
		if(!$allowTags) $str = strip_tags($str);
