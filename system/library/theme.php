...
class theme
{
	...
	public function view($view,$data=NULL)
	{
		...
		if(file_exists(APPLICATION."/views/{$this->current}/$view.php"))
		{
			$this->dingo->load->view("{$this->current}/$view",$data);
		}
		
		// If view not found in theme try to load from default theme
		elseif($this->current != 'default' AND file_exists(APPLICATION."/views/default/$view.php"))
		{
		...
		{
			dingoError(E_USER_WARNING,'The requested theme view ('.APPLICATION."/views/{$this->current}/$view.php) could not be found.");
		}
	}
}
